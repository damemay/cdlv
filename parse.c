...
static inline cdlv_error parse_global_resource(cdlv* base, cdlv_parse_mode* mode, const char* line) {
    ...
    printf("\t-- extracting");
    cdlv_error res;
    if((res = add_new_resource(base, base->resources_path, line, base->resources)) != cdlv_ok) {
        cdlv_err(res);
    }
    cdlv_err(cdlv_ok);
}
...
static inline cdlv_error parse_scene_resource(cdlv* base, cdlv_parse_mode* mode, const char* line, cdlv_scene** scene) {
    ...
    printf("\t-- extracting");
    cdlv_error res;
    if((res = add_new_resource(base, current_scene->resources_path, line, current_scene->resources)) != cdlv_ok) {
        cdlv_err(res);
    }
    cdlv_err(cdlv_ok);
}
