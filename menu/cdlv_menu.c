#include "../cdlv.h"
#include "../cdlv_util.h"
#include "cdlv_menu.h"
#include "../zocket/zocket.h"
#include <dirent.h>

static inline void menu_reprint(cdlv_menu* menu) {
    if(menu->current_choice < menu->file_count) {
        strcpy(menu->text, menu->path);
        strcat(menu->text, "\n" cdlv_arrow);
        for(size_t i=menu->current_choice; i<menu->file_count; ++i) {
            strcat(menu->text, menu->files[i]);
            strcat(menu->text, "\n");
        }
        strcat(menu->text, " ");
    }
}

cdlv_menu* cdlv_menu_create(cdlv_base* base, const char* path, sdl_base* sdl, int fd) {
    cdlv_menu* menu = malloc(sizeof(cdlv_menu));
    if(!menu)
        cdlv_die("Could not allocate memory for the menu");

    char* cmd = "LIST";
    zkt_data_send_compress(fd, cmd, strlen(cmd)+1, 90);
    zkt_log("sent cmd LIST. awaiting response...");
    zkt_data* files = zkt_data_recv(fd);
    menu->path_exists = true;
    menu->file_count = 0;
    cdlv_duplicate_string(&menu->path, path, strlen(path)+1);
    cdlv_alloc_ptr_arr(&menu->files, cdlv_max_menu_entries, char*);

    char* tok = strtok(files->buffer, "\r\n");
    while(tok) {
        menu->files[menu->file_count] = calloc(strlen(tok)+1, sizeof(char));
        strcpy(menu->files[menu->file_count], tok);
        ++menu->file_count;
        tok = strtok(NULL, "\r\n");
    }

    cdlv_text_create(base,
            base->config->text_font, 16, 700,
            10, 10,
            255, 255, 255, 255, sdl->renderer);

    cdlv_alloc_ptr_arr(&menu->text, cdlv_max_string_size, char);
    if(menu->path_exists) {
        menu->current_choice = 0;
        menu_reprint(menu);
    } else {
        sprintf(menu->text, "Folder %s not found!", path);
    }

    menu->text_speed = base->config->text_speed;
    base->config->text_speed = 0;
    cdlv_text_update(base, menu->text);

    return menu;
}

void cdlv_menu_render(cdlv_base* base, sdl_base* sdl) {
    SDL_RenderClear(sdl->renderer);
    cdlv_text_render(base, sdl->renderer);
}

void cdlv_menu_clean(cdlv_menu* menu) {
    for(size_t i=0; i<menu->file_count; ++i)
        free(menu->files[i]);
    free(menu->files);
    free(menu->text);
    free(menu->path);
    free(menu);
}

int check_errors(cdlv_base* base, cdlv_error err) {
    if(err == cdlv_no_err) return 0;
    switch(err) {
        case cdlv_config_err:
            cdlv_text_update(base, base->log); return -1;
        case cdlv_no_mem_err:
            cdlv_text_update(base, base->log); return -1;
        case cdlv_file_err:
            cdlv_text_update(base, base->log); return -1;
        case cdlv_script_err:
            cdlv_text_update(base, base->log); return -1;
        case cdlv_sdl_err:
            cdlv_text_update(base, base->log); return -1;
        default:return -1;
    }
}

static inline void menu_load_script(cdlv_base** base, cdlv_menu** menu, sdl_base* sdl, zkt_client** client) {
    if((*menu)->current_choice >= 0 && (*menu)->current_choice < (*menu)->file_count) {
        zkt_vlog("%s", (*menu)->files[(*menu)->current_choice]);
        (*base)->config->text_speed = (*menu)->text_speed;
        cdlv_config* config = (*base)->config;
        if(check_errors(*base, (*base)->error) == -1) return;
        cdlv_clean_all((*base));
        *base = NULL;
        *base = cdlv_create(config);

        zkt_client_reconnect(client);
        char cmd[cdlv_max_string_size];
        sprintf(cmd, "GET %s", (*menu)->files[(*menu)->current_choice]);
        zkt_data_send_compress((*client)->fd, cmd, strlen(cmd)+1, 90);
        zkt_vlog("sent command %s...", cmd);
        zkt_data* script = zkt_data_recv((*client)->fd);
        cdlv_read_file((*base), script->buffer, script->size,(*menu)->files[(*menu)->current_choice], &sdl->renderer);
        if(check_errors(*base, (*base)->error) == -1) return;
        SDL_SetWindowSize(sdl->window, (*base)->canvas->w, (*base)->canvas->h);
        cdlv_start((*base), client);
        if(check_errors(*base, (*base)->error) == -1) return;
        cdlv_menu_clean((*menu));
        *menu = NULL;
        (*base)->state = cdlv_main_run;
    }
}

void cdlv_menu_handle_keys(cdlv_base** base, cdlv_menu** menu, sdl_base* sdl, zkt_client** client) {
    if((*base)->can_interact) {
        if(sdl->event.type == SDL_KEYUP && sdl->event.key.repeat == 0) {
            switch(sdl->event.key.keysym.sym) {
                case SDLK_UP:
                    if((*menu)->current_choice > 0) {
                        --(*menu)->current_choice;
                        menu_reprint(*menu);
                        cdlv_text_update((*base), (*menu)->text);
                    }
                    break;
                case SDLK_DOWN:
                    if((*menu)->current_choice < (*menu)->file_count) {
                        ++(*menu)->current_choice;
                        menu_reprint((*menu));
                        cdlv_text_update((*base), (*menu)->text);
                    }
                    break;
                case SDLK_RETURN: menu_load_script(base, menu, sdl, client); break;
            }
        }
        if(sdl->event.type == SDL_CONTROLLERBUTTONUP) {
            switch(sdl->event.cbutton.button) {
                case SDL_CONTROLLER_BUTTON_DPAD_UP:
                    if((*menu)->current_choice > 0) {
                        --(*menu)->current_choice;
                        menu_reprint(*menu);
                        cdlv_text_update((*base), (*menu)->text);
                    }
                    break;
                case SDL_CONTROLLER_BUTTON_DPAD_DOWN:
                    if((*menu)->current_choice < (*menu)->file_count) {
                        ++(*menu)->current_choice;
                        menu_reprint((*menu));
                        cdlv_text_update((*base), (*menu)->text);
                    }
                    break;
                case SDL_CONTROLLER_BUTTON_A: menu_load_script(base, menu, sdl, client); break;
            }
        }
    }
}
